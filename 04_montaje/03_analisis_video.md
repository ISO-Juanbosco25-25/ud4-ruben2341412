# 🔍 ANÁLISIS DEL VÍDEO DE MONTAJE

## 👨‍💻 Alumno:ruben de la guia 
## 🖥️ Equipo:

---

## 🎯 OBJETIVO

Analizar de forma crítica el proceso de montaje del equipo a partir del vídeo realizado, identificando aciertos, errores y aspectos mejorables.

---

## 🎬 VÍDEO ANALIZADO

📌 Enlace al vídeo: https://youtu.be/dgFyisJ-7WM?feature=shared

---

# 🔎 ANÁLISIS POR FASES

---

## 🧩 Fase 1: Preparación

📌 Minuto del vídeo:00:01 mas o menos hasta 00:45

📌 ¿Qué se hace en esta fase?
preparar las herramientas y dejar todo limpio para que no se pueda caer ni derramar nada 

📌 ¿Se realiza correctamente? (Sí / No / Parcialmente)
si 
📌 Observaciones:
 Organización del espacio:  esta todo a mano y el area esta despejada
Uso de herramientas: utilizamos las herramientas necesarias y que hay 
Seguridad (electricidad estática, etc.): es bastante seguro ya que no esta enchufado ala electricidad 

📌 Valoración técnica:
(Explica si está bien hecho y por qué)
De diez. Empezar con buen orden es la mitad del trabajo para no volverte loco luego buscando tornillos.
---

## 🧩 Fase 2: Instalación de componentes principales

(CPU, RAM, placa base)

📌 Minuto del vídeo: 20:00 AL MINUTO :22:00

📌 ¿Qué se hace?
Pinchar el procesador en su sitio, ponerle la pasta térmica y encajar la RAM.
📌 ¿Se realiza correctamente?
si
📌 Problemas observados:Ninguno, todo ha ido perfecto 

📌 Riesgos detectados:el error que tuve fue que no presionaba bien y me costo un poco 
(Ejemplo: mala colocación, presión incorrecta, etc.]


📌 Valoración técnica:
Muy bien. El procesador ha entrado perfecto y la RAM ha hecho el "clic" que da esa tranquilidad de que está bien puesta y un peuqeño fallo que al poner la ram me costo un poco 
---

## 🧩 Fase 3: Instalación de almacenamiento

📌 Minuto del vídeo:DESDE EL 22:30 AL 28:45

📌 ¿Qué se hace?
Atornillar el SSD a la torre para que no ande bailando por ahí.

📌 ¿Se realiza correctamente?
si
📌 Observaciones:ninguna todo perfecto

📌 Valoración técnica:
Fácil y limpio. El disco está bien sujeto y los cables no estorban.
---

## 🧩 Fase 4: Cableado

📌 Minuto del vídeo:
29:00 AL 35:00
📌 ¿Qué se hace?
conectar todos los cables de la fuente y los botoncitos de la torre a la placa.
📌 ¿Se realiza correctamente?
si

📌 Problemas detectados:
Aunque hay bastantes cables por el medio, lo importante es que todos han llegado a su sitio y están bien apretados.
📌 Valoración técnica:
Todo en orden. Hes conectado bien el cable gordo de la placa y el de la CPU, que es el que más guerra da a veces.
---

## 🧩 Fase 5: Encendido y comprobación

📌 Minuto del vídeo:
en el final de video sobre el minuto 37
📌 ¿Arranca el equipo?
si
📌 Problemas detectados:
ninguno 
📌 Comprobaciones realizadas:
que enciende el ordenador 
📌 Valoración técnica:
si enciende el ordenador pues se supone que va 
---

# ⚖️ VALORACIÓN GLOBAL DEL MONTAJE

## ✔️ Aciertos

Cero fallos con la CPU: No he doblado nada, que es el mayor miedo de cualquier persona.

RAM bien puesta: He aprovechado el máximo rendimiento desde el minuto uno.

Sin líos de estática: El equipo ha arrancado perfecto, así que el manejo ha sido seguro.


## ❌ Fallos detectados
- error ala colocacion del panel frontal 
- ala hora de colocar el disipador me costo bastante ya que el disipador estab aun poco roto 
  


---

## ⚠️ Riesgos potenciales
- la falta de componentes que hace que no funciones el pc 
- 
- 

---

# 🧠 REFLEXIÓN FINAL

- ¿El montaje es correcto en general? Justifica.
  Sí, totalmente. El PC funciona, que es lo que cuenta, y no he cometido ningún error que ponga en peligro las piezas.

- ¿Crees que este equipo funcionaría a largo plazo sin problemas? ¿Por qué?
si por que esta hecho con delicadeza y todo lo que habia en el pc esta puesto bien 
- Si fueras técnico en una empresa, ¿darías este equipo como válido o lo revisarías?
 si lo daria como valido 
